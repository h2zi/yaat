...
Yet Another Account Tool：面向 Codex、Claude Code 与 Claude Desktop 的本地多账号 / 多 Provider 管理工具
...
Codex 只接管模型、Provider 选择、`model_providers.custom` 和可选 `model_catalog_json`。官方与第三方配置统一使用 `custom`，本期只生成 Responses wire API；第三方凭据直接写入 `experimental_bearer_token`。TOML 对象使用显式多行表，不会压成一整行 inline table。`cli_auth_credentials_store` 在全局切换中保持用户现有选择；仅 YAAT 自己生成的隔离 `CODEX_HOME` 明确使用 `file`，让每个 Profile 拥有独立的 `auth.json`。

Claude Code 的 `settings.json` 只接管 `env` 中的 Anthropic Provider 选择、直接凭据、自定义请求头和模型映射字段；激活时会清理 YAAT 旧版遗留的 `apiKeyHelper`。默认、Sonnet、Opus、Haiku、Fable 与 SubAgent 可分别映射。`permissions`、MCP、插件、主题、沙箱规则、用户注释和其他未知字段都不属于 YAAT。官方账号快照也不是整份 secure storage：只包含 `claudeAiOauth`、`organizationUuid`、`trustedDeviceToken`、`enterpriseGateway` 四个账号字段。

Claude Desktop 的官方账号按 Profile 设置独立 `CLAUDE_USER_DATA_DIR`。第三方 Provider 使用 Desktop 原生 3P config-library，YAAT 直接写入 inference gateway 凭据、认证类型、自定义请求头、User-Agent 和直连模型，不再生成 helper wrapper。真正的任意上游模型角色映射需要本地协议路由，留到后续路由版本实现。
...
- 第三方 API Key / Token 直接写入客户端原生配置字段；YAAT 不再生成或调用凭据助手。日志、错误、模型拉取响应和普通 IPC 返回值不会输出凭据。
...
- 源文件按路径、大小、mtime 和内容指纹增量索引；同一逻辑事件即使出现在多个复制历史中也只统计一次。Codex 同时支持普通 `.jsonl` 和 `.jsonl.zst`；单个源文件超过 512 MiB 时跳过并标记为部分统计。
- 日期查询最长 366 天，按用户选择的 IANA 时区聚合；支持按模型过滤、自动刷新、缓存命中 Token 与缓存命中率。

## 会话历史统一

会话历史入口位于设置中，所有写操作都要求对应客户端已经退出。预览用于展示计划，应用时会重新扫描当前文件。

- Codex：首次统一和“重新统一”会把 JSONL / Zstd 中两处 Provider 元数据以及 `state_5.sqlite.threads.model_provider` 全部归一为 `custom`；启用后不可关闭。全局目录和各托管 `CODEX_HOME` 之间增量同步缺失或严格扩展的会话，真正分叉时报告冲突且不覆盖。
...
- 设置中的自动同步默认关闭。账号激活和隔离启动只投递后台增量任务，不在“切换成功”返回前枚举历史目录；同步失败只更新任务状态，不撤销已完成的账号操作。Claude Code 关闭后只停止后续同步，不删除已复制会话。Claude Desktop 必须先保存明确的目标账号 / 组织。
...
# YAAT 的严格配置、直接 Provider 凭据和官方账号私有凭据复制切换
...
Codex 互操作镜像固定安装官方 `@openai/codex 0.147.0`。测试源码不挂载宿主机账号目录；容器运行阶段使用 `--network none`、移除全部 capabilities，并验证多行 `custom` Provider、Responses、直接凭据与官方账号私有凭据复制切换，同时确认用户自有 Provider、MCP 配置、注释和非账号凭据字段未丢失。

Claude 互操作镜像固定安装官方 `@anthropic-ai/claude-code 2.1.226`，直接复用生产 Claude 适配器生成全局与隔离 `settings.json`，验证直接凭据、自定义请求头、注释、permissions、MCP 和未知字段保留，并让真实 CLI 在断网容器中加载同一隔离环境。这些固定版本是可重复的互操作测试基准，不是运行时版本门禁。
